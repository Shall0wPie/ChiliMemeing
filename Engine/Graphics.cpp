...
void Graphics::MakeTriangle(Vertex *vertArr, int size)
{
	int f = sizeof(vertArr);
	for (int i = 0; i < size-2; i++)
	{
		MakeLine(vertArr[i].x, vertArr[i].y, vertArr[i + 1].x, vertArr[i + 1].y);
		MakeLine(vertArr[i].x, vertArr[i].y, vertArr[i + 2].x, vertArr[i + 2].y);
		MakeLine(vertArr[i + 1].x, vertArr[i + 1].y, vertArr[i + 2].x, vertArr[i + 2].y);
	}
}

void Graphics::MakeRect(int x, int y, int xSize, int ySize, Color color)
{
